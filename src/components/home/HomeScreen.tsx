  ...
  return (
    <div className="flex flex-col flex-1 pb-24 select-none">
      ...
      <div className="p-3 sm:p-4 max-w-lg mx-auto w-full">
        {/* Calendar Card */}
        <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200/80 dark:border-zinc-800 shadow-xs p-3.5 sm:p-4">
          {/* Calendar Month Header with jump selector */}
          <div className="flex items-center justify-between mb-3 px-1">
            <button
              onClick={() => setShowMonthPicker(prev => !prev)}
              className="flex items-center gap-1.5 text-base font-extrabold text-zinc-900 dark:text-zinc-100 hover:text-emerald-600 dark:hover:text-emerald-400 transition cursor-pointer"
            >
                ...
                <button
                  ...
                  className={`relative aspect-square rounded-xl flex flex-col items-center justify-between p-1 transition cursor-pointer ${
                    !cell.isCurrentMonth
                      ? 'opacity-25'
                      : cell.isFuture
                      ? 'bg-zinc-50/50 dark:bg-zinc-800/30 text-zinc-400 dark:text-zinc-600'
                      : isSelected
                      ? 'ring-2 ring-emerald-500 bg-emerald-50/60 dark:bg-emerald-950/30 font-bold'
                      : cell.isToday
                      ? 'border border-emerald-400 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 font-bold'
        ...
        {/* Selected Date Quick Summary Card */}
        {/* Rule: "Tapping a date first opens a quick summary, with an obvious action to open that date’s full detail/history." */}
        <div className="mt-3 bg-white dark:bg-zinc-900 rounded-2xl p-3.5 border border-zinc-200/80 dark:border-zinc-800 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              ...
              {selectedDate === todayDate && (
                <span className="px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300">
                  Today
                </span>
              )}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            {selectedDateSummary.completedCount > 0 ? (
              <div className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 font-semibold text-[11px]">
                <CheckCircle2 size={12} />
                <span>{selectedDateSummary.completedCount} completed</span>
              </div>
            ) : (
              <div className="px-2.5 py-1 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 font-medium text-[11px]">
                0 completed
              </div>
            )}
            ...
            {selectedDateSummary.completedHabits.map(h => (
              <div
                key={h.id}
                onClick={() => onOpenHabitDetail(h.id)}
                className="flex items-center gap-1 px-2 py-0.5 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-[11px] font-medium text-zinc-800 dark:text-zinc-200 cursor-pointer hover:border-emerald-400"
              >
            ...
            {selectedDateSummary.hasFreeze && (
              <div className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 font-semibold text-[11px]">
                <Shield size={12} />
